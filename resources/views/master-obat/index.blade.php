<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola data referensi obat untuk form pemeriksaan (Khusus Super Admin Dinkes).</p>
            </div>
            <div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nama Obat</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($obats as $obat)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-semibold text-slate-900 dark:text-white">{{ $obat->nama_obat }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onclick="editObat('{{ $obat->id }}', '{{ $obat->nama_obat }}')" class="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-400" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <form action="{{ route('master-obat.destroy', $obat->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus obat ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h4 class="text-sm font-semibold text-slate-900 dark:text-white">Belum Ada Data Obat</h4>
                                </div>
                            </td>
                        </tr>
                    @endempty
    ...
    <div id="addModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100 dark:border-slate-700">
            ...
            <form action="{{ route('master-obat.store') }}" method="POST" class="p-5">
                @csrf
                <div class="mb-4">
    ...
    <script>
        function editObat(id, nama) {
            ...
            document.getElementById('editForm').action = '/master-obat/' + id;
