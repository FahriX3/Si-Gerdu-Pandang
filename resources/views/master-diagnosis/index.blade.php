<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">
                    {{ __('Data Master Diagnosis') }}
                </h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola data referensi diagnosis untuk form pemeriksaan (Khusus Super Admin Dinkes).</p>
            </div>
            <div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nama Diagnosis</th>
                    ...
                    @forelse ($diagnoses as $diagnosis)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                            <td class="px-6 py-4 text-sm font-medium text-slate-500 dark:text-slate-400">
                                {{ $loop->iteration + $diagnoses->firstItem() - 1 }}
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-semibold text-slate-900 dark:text-white">{{ $diagnosis->nama_diagnosis }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onclick="editDiagnosis('{{ $diagnosis->id_diagnosis }}', '{{ addslashes($diagnosis->nama_diagnosis) }}')" class="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-400" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <form action="{{ route('master-diagnosis.destroy', $diagnosis->id_diagnosis) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus diagnosis ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h4 class="text-sm font-semibold text-slate-900 dark:text-white">Belum Ada Data Diagnosis</h4>
                                </div>
                            </td>
                        </tr>
                    @endempty
    ...
    <div id="addModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100 dark:border-slate-700">
            <div class="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Tambah Diagnosis Baru</h3>
            ...
            <form action="{{ route('master-diagnosis.store') }}" method="POST" class="p-5">
                @csrf
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Nama Diagnosis</label>
                    <input type="text" name="nama_diagnosis" placeholder="Contoh: HT Terkontrol" required class="bg-white border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                </div>
    ...
    <div id="editModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100 dark:border-slate-700">
            <div class="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Edit Diagnosis</h3>
            ...
            <form id="editForm" method="POST" class="p-5">
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Nama Diagnosis</label>
                    <input type="text" name="nama_diagnosis" id="edit_nama_diagnosis" required class="bg-white border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                </div>
                <div class="flex justify-end gap-3 mt-6">
    ...
    <script>
        function editDiagnosis(id, nama) {
            document.getElementById('edit_nama_diagnosis').value = nama;
            document.getElementById('editForm').action = '/master-diagnosis/' + id;
