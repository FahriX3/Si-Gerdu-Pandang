<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">
                    {{ __('Data Master Dukuh/Dusun') }}
                </h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola data referensi Dukuh/Dusun berdasarkan Kalurahan (Khusus Super Admin Dinkes).</p>
            </div>
            <div>
    ...
    <div class="space-y-6 relative z-10">
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 sm:rounded-2xl overflow-hidden">
                <div class="bg-slate-50/50 dark:bg-slate-900/50 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                        ...
                        Kalurahan: {{ $nama_kelurahan }}
                    </h3>
                    <span class="px-3 py-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-full">{{ count($listDukuh) }} Dukuh</span>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        @foreach($listDukuh as $dukuh)
                            <div class="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-primary-400 dark:hover:border-primary-600 hover:shadow-md transition-all group">
                                <span class="font-medium text-slate-700 dark:text-slate-200">{{ $dukuh->nama_dukuh }}</span>
                                <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onclick="editDukuh('{{ $dukuh->id_dukuh }}', '{{ $dukuh->id_kelurahan }}', '{{ addslashes($dukuh->nama_dukuh) }}')" class="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors dark:hover:bg-blue-900/50 dark:text-blue-400" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <form action="{{ route('master-dukuh.destroy', $dukuh->id_dukuh) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus Dukuh {{ addslashes($dukuh->nama_dukuh) }}?');">
                                        @csrf
                                        @method('DELETE')
        ...
        @empty
            <div class="bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 sm:rounded-2xl p-12 text-center">
                ...
                <h4 class="text-sm font-semibold text-slate-900 dark:text-white">Belum Ada Data Dukuh</h4>
            </div>
        @endforelse
    </div>
    ...
    <div id="addModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100 dark:border-slate-700">
            <div class="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Tambah Dukuh Baru</h3>
            ...
            <form action="{{ route('master-dukuh.store') }}" method="POST" class="p-5">
                @csrf
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Pilih Kalurahan</label>
                    <select name="id_kelurahan" required class="bg-white border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Nama Dukuh/Dusun</label>
                    <input type="text" name="nama_dukuh" placeholder="Contoh: Kledekan Kidul" required class="bg-white border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                </div>
    ...
    <div id="editModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100 dark:border-slate-700">
            <div class="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Edit Dukuh</h3>
            ...
            <form id="editForm" method="POST" class="p-5">
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Pilih Kalurahan</label>
                    <select name="id_kelurahan" id="edit_id_kelurahan" required class="bg-white border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                        <option value="">-- Pilih Kalurahan --</option>
                        @foreach($kelurahans as $kel)
                            <option value="{{ $kel->id_kelurahan }}">{{ $kel->nama_kelurahan }}</option>
                        @endforeach
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Nama Dukuh/Dusun</label>
                    <input type="text" name="nama_dukuh" id="edit_nama_dukuh" required class="bg-white border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white">
                </div>
                <div class="flex justify-end gap-3 mt-6">
    ...
    <script>
        function editDukuh(id, idKelurahan, dukuh) {
            document.getElementById('edit_id_kelurahan').value = idKelurahan;
            document.getElementById('edit_nama_dukuh').value = dukuh;
            document.getElementById('editForm').action = '/master-dukuh/' + id;
